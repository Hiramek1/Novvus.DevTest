...
const challenges = {
  processarPedidos: {
    ...
    hints: [
      "Utilize um Set para armazenar clientes únicos.",
      "Itere sobre a lista de pedidos e atualize os totais.",
      "Certifique-se de verificar a data dos pedidos para incluir apenas os recentes."
    ],
    testCases: [
      {
        input: [
  ...
  dijkstra: {
    ...
    hints: [
      "Inicialize distâncias com infinito, exceto para o nó de origem.",
      "Use uma fila de prioridade para processar os nós mais próximos primeiro.",
      "Atualize a distância dos vizinhos se encontrar um caminho mais curto."
    ],
    testCases: [
      {
        input: {
...
// Inicializa o botão para mostrar dicas
document.getElementById('showHints').addEventListener('click', () => {
  const selectedChallenge = document.querySelector('input[name="challenge"]:checked').value;
  const hints = challenges[selectedChallenge].hints;
  const hintsHtml = hints.map(hint => `<li>${hint}</li>`).join('');
  document.getElementById('hints').innerHTML = `<ul>${hintsHtml}</ul>`;
});
...
document.querySelector('input[name="challenge"]:checked').dispatchEvent(new Event('change'));